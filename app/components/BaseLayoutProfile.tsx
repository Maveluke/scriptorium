...
export default function BaseLayoutProfile({ children, user }: BaseLayoutPropsProfile) {
  ...
  return (
    <div className={`flex ${isDarkMode ? 'bg-slate-900' : 'bg-slate-300'} min-h-screen`}>
      ...
      {/* Left sidebar */}
      <div
        className={`fixed left-0 top-0 h-screen w-64 bg-slate-900 border-r border-slate-800 z-50 transition-transform duration-300 transform ${isSideNavOpen ? "translate-x-0" : "-translate-x-full"
          }`}>
